...
class ImmoController extends Controller
{
    ...
    public function researchInList(ResearchInListRequest $request,  ImmoRepository $immoRepository)
    {
        ...
        foreach ($listProperties as $key => $property) {
            $picture = $immoRepository->getMainPictureByIdProperty($property->idProperty);
            $listProperties[$key]->urlPicture = '/uploads/property-' . $property->idProperty . '/pic-' . $picture->id . '.' . $picture->extension;
            $listProperties[$key]->fk_energy_class = 'img/peb/peb_' . $property->fk_energy_class . '.png';
        }
        return [
    ...
    public function getListProperties(ResearchInListRequest $request, ImmoRepository $immoRepository){
        ...
        return view('listingOfProperties', [
            'req' => $request->input(),
            'sell_or_rent' => $immoRepository->getSellOrRent(),
            "property_sub_type" => $listPropertiesSubType
        ]);
    }
